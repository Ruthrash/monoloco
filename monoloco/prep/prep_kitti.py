...
class PreprocessKitti:
    ...
    def __init__(self, dir_ann, mode='mono', iou_min=0.3):
        ...
        self.dir_ann = dir_ann #annotations
        self.iou_min = iou_min # = 0.3
        self.mode = mode
        assert self.mode in ('mono', 'stereo'), "modality not recognized"
        self.names_gt = tuple(os.listdir(self.dir_gt)) 
        self.dir_kk = os.path.join('data', 'kitti', 'calib')#fixed
    ...
    def run(self):
        ...
        for name in self.names_gt:
            ...
            cnt_gt[phase] += len(boxes_gt)# total ground truth boxes
            cnt_files += 1
            cnt_files_ped += min(len(boxes_gt), 1)  # if no boxes 0 else 1

            # Extract keypoints
            path_calib = os.path.join(self.dir_kk, basename + '.txt') ##gets calibration
            annotations, kk, tt = factory_file(path_calib, self.dir_ann, basename) ### -----> what is this?

            self.dic_names[basename + '.png']['boxes'] = copy.deepcopy(boxes_gt) # gt
            self.dic_names[basename + '.png']['ys'] = copy.deepcopy(ys) #?
            self.dic_names[basename + '.png']['K'] = copy.deepcopy(kk) ##calibration

            # Check image size
            with Image.open(path_im) as im:
                width, height = im.size

            boxes, keypoints = preprocess_pifpaf(annotations, im_size=(width, height), min_conf=min_conf)#### gets boxes and keypoints from pipaf

            if keypoints:
                annotations_r, kk_r, tt_r = factory_file(path_calib, self.dir_ann, basename, mode='right')#### -----> what is this?
                boxes_r, keypoints_r = preprocess_pifpaf(annotations_r, im_size=(width, height), min_conf=min_conf)
                cat = get_category(keypoints, os.path.join(self.dir_byc_l, basename + '.json'))###3 what is this?
